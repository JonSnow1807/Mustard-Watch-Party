---- MODULE SyncTimeline_TTrace_1785773356 ----
...
\* to `JsonSerialize`. For example, a sub-sequence of _TETrace.
    \* ASSUME
    \*     LET J == INSTANCE Json
    \*         IN J!JsonSerialize("SyncTimeline_TTrace_1785773356.json", _TETrace)
...
\*
\*trace == IODeserialize("SyncTimeline_TTrace_1785773356.bin", TRUE)
\*
\*=============================================================================
\*
...
trace == 
    ...
    ([deadEpochs |-> (c1 :> {} @@ c2 :> {}),applied |-> (c1 :> [epoch |-> 2, seq |-> 1] @@ c2 :> [epoch |-> 0, seq |-> 0]),storeSeq |-> 1,storeEpoch |-> 2,network |-> {[epoch |-> 1, seq |-> 1, dst |-> c1], [epoch |-> 1, seq |-> 1, dst |-> c2], [epoch |-> 2, seq |-> 1, dst |-> c2]}]),
    ([deadEpochs |-> (c1 :> {} @@ c2 :> {}),applied |-> (c1 :> [epoch |-> 1, seq |-> 1] @@ c2 :> [epoch |-> 0, seq |-> 0]),storeSeq |-> 1,storeEpoch |-> 2,network |-> {[epoch |-> 1, seq |-> 1, dst |-> c2], [epoch |-> 2, seq |-> 1, dst |-> c2]}])
    >>
----
...
---- CONFIG SyncTimeline_TTrace_1785773356 ----
CONSTANTS
    Clients = { c1 , c2 }
    MaxSeq = 3
    MaxEpoch = 2
...
ALIAS
    _expression
=============================================================================
\* Generated on Mon Aug 03 11:09:16 CDT 2026